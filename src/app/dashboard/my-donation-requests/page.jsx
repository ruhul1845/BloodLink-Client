...
export default function MyDonationRequestsPage() {
  ...
  const load = useCallback(async () => {
    setRows(await api(`/api/requests/my${status ? `?status=${status}` : ""}`));
  }, [status]);
  useEffect(() => { if (user) load(); }, [user, load]);
  return (
    <section>
      <div className="flex flex-wrap items-center justify-between gap-4">
        <h1 className="text-4xl font-black">My Donation Requests</h1>
      ...
      <div className="mt-8"><RequestTable rows={rows} refresh={load} role={user?.role} /></div>
    </section>
  );
}
